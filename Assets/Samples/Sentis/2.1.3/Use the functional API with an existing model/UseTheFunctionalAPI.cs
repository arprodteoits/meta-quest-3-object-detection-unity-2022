...
public class UseTheFunctionalAPI : MonoBehaviour
{
    ...
    [SerializeField]
    Unity.InferenceEngine.ModelAsset sourceModelAsset;

    Unity.InferenceEngine.Model m_RuntimeModel;
    ...
    void OnEnable()
    {
        ...
        var sourceModel = Unity.InferenceEngine.ModelLoader.Load(sourceModelAsset);
        ...
        var sRGB = Unity.InferenceEngine.Functional.Pow(RGB, Unity.InferenceEngine.Functional.Constant(1 / 2.2f));
        ...
        var outputs = Unity.InferenceEngine.Functional.Forward(sourceModel, sRGB_normalised);
        ...
        using var input = Unity.InferenceEngine.TextureConverter.ToTensor(inputTexture, 64, 64, 3);
        ...
        var output = m_Worker.PeekOutput("output_0") as Unity.InferenceEngine.Tensor<float>;
